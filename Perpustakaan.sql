...
RETURNS TRIGGER AS $$
BEGIN
    -- Cek apakah jumlah yang dipinjam melebihi stok buku
    IF (SELECT stok FROM buku WHERE id_buku = NEW.id_buku) < NEW.jumlah_yang_dipinjam THEN
        RAISE EXCEPTION 'Jumlah yang dipinjam melebihi stok buku';
    ELSE
